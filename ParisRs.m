...
%RS priority---------
RsPrio = round(poissrnd(0.1,1,2000));%randi([0 1],1,2000);
%--------------------------
...
if test ==0
    for i=1:parknb
cap2(i)= str2num( traci.simulation.getParameter(num2str(i),'parkingArea.capacity'));
ResPl{i}=round((cap(i)*5)/100);
    end
             test=nnz(RsPrio);
...
[EstAv,ResPl]=GetFreeSpace(EstAv,cap,step,parknb,ResPl);
...
%-------------------------------------- ------------------
%Sr = RevTravTime(vehinsim,departVeh,parknb,ParkEdgeID );
%---------------Starting implimenting the RS-----!!!!
[AddT,sub1,ResPl,A,RsPrio]=RewardSys(tempV,departVeh,DF,vehinsim,vehicles,AddT,Dist,sub1,A,RsPrio,ResPl,ParkEdgeID,parknb,ConvtimeV,step);
%----------------------------------------------------------------
%----------------------------
for c=1: vehinsim-1%traci.simulation.getDepartedNumber()
    v=departVeh{1,c};
...
%       end
  if ((sub1(j)==i))&&(A(j)==0)&&ismember(v,vehicles)%&&~strcmp(pedg,vedg)
    ...
    s=1/mu(i)*10000;
     pedg=ParkEdgeID{i};
    if cap(i)-nnz(EstAv{i})>nnz(ResPl{i})+3%1-ParkAv(i)>0%
